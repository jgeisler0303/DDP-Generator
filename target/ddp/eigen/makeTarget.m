...
if exist(fullfile(env.build_dir, 'ddp_problem.h'), 'file') && (~exist('force_gen', 'var') || ~force_gen) && ~last_err
    dd_src= dir(env.problem_file);
    dd_tgt= dir(fullfile(env.build_dir, 'ddp_problem.h'));
...
if ~skip_gen
    if ispc
        command_line= ['-q --batch-string="batchload(\"make_ddp.mac\")\$ gen_ddp(\"', strrep(env.problem_file, '\', '\\'), '\", \"', strrep(env.build_dir, '\', '\\'), '\")\$" 2>', env.maxima_errlog_file];
    else
        command_line= ['-q --batch-string="load(\"make_ddp.mac\")\$ gen_ddp(\"', env.problem_file, '\", \"', env.build_dir, '\")\$" 2>', env.maxima_errlog_file];
    end
    ...
    if ~maxima_res
        mex_name= '';
        if exist(fullfile(env.target_dir, 'ddp_problem.h'), 'file')
            movefile(fullfile(env.target_dir, 'ddp_problem.h'), fullfile(env.target_dir, 'faulty_ddp_problem.h')); 
        end
        if exist(fullfile(env.target_dir, 'ddp_func.cpp'), 'file')
            movefile(fullfile(env.target_dir, 'ddp_func.cpp'), fullfile(env.target_dir, 'faulty_ddp_func.cpp'));
        end
...
compile_files{end+1}= 'ddp_mex.cpp';
...
compile_files{end+1}= fullfile(env.build_dir, 'ddp_func.cpp');
