...
#if 0
    #define PR(s) mexPrintf(s)
    #define PR1(s, x1) mexPrintf(s, x1)
    #define PR2(s, x1, x2) mexPrintf(s, x1, x2)
    #define PR3(s, x1, x2, x3) mexPrintf(s, x1, x2, x3)
#else
    #define PR(s) {}
    #define PR1(s, x1) {}
    #define PR2(s, x1, x2) {}
...
//		if(tmp_max<taubar*gamma || tmp_min<tau*tmp_max) {
		...
		if(tmp_max<taubar*gamma || tmp_min<-mu*tmp_max) {
			...
			if(id!=j) {
				//switch rows and cols of id and j of L_
...
//			if(tmp_min<tau*gamma) {
	...
	/* Phase two, L_ not positive-definite */
	if(!phaseone && (j==(n-1))) {
...
void chol_inv(double *L_, int *P, double *invA, int n) {
    ...
    for(l= 0; l<n; l++) {
        ...
        for(k= n-1; k >= l; k--) {
            ...
            if(rp!=cp) invA[MAT_IDX(cp, rp, n)]= invA[MAT_IDX(rp, cp, n)];
        }
    }
    free(x);
}
